  ...
  */
  class Datatables
  {
    ...
    */
    ...
    {
			$iStart = $this->ci->input->get_post('iDisplayStart');
      $iLength = $this->ci->input->get_post('iDisplayLength');
      $this->ci->db->limit(($iLength != '' && $iLength != '-1')? $iLength : 25, ($iStart)? $iStart : 0);
    }
    ...
    */
    protected function get_ordering()
    {
      ...
      elseif($this->ci->input->get_post('sColumns'))
        $mColArray = explode(',', $this->ci->input->get_post('sColumns'));
      else
      ...
      for($i = 0; $i < intval($this->ci->input->get_post('iSortingCols')); $i++)
        if(isset($mColArray[intval($this->ci->input->get_post('iSortCol_' . $i))]) && in_array($mColArray[intval($this->ci->input->get_post('iSortCol_' . $i))], $columns) && $this->ci->input->get_post('bSortable_'.intval($this->ci->input->get_post('iSortCol_' . $i))) == 'true')
          $this->ci->db->order_by($mColArray[intval($this->ci->input->get_post('iSortCol_' . $i))], $this->ci->input->get_post('sSortDir_' . $i));
    }
    ...
    */
    protected function get_filtering()
    {
      ...
      $sWhere = '';
      $sSearch = trim(strip_tags($this->ci->input->get_post('sSearch')));
      $mColArray = array_values(array_diff($mColArray, $this->unset_columns));
      $columns = array_values(array_diff($this->columns, $this->unset_columns));

      if($sSearch != '')
        for($i = 0; $i < count($mColArray); $i++)
          if($this->ci->input->get_post('bSearchable_' . $i) == 'true' && in_array($mColArray[$i], $columns))
      ...
      for($i = 0; $i < intval($this->ci->input->get_post('iColumns')); $i++)
      {
        if(isset($_POST['sSearch_' . $i]) && $this->ci->input->get_post('sSearch_' . $i) != '' && in_array($mColArray[$i], $columns))
        {
          $miSearch = explode(',', $this->ci->input->get_post('sSearch_' . $i));
    ...
    */
    protected function produce_output($charset)
    {
      ...
      $sOutput = array
      (
        'sEcho'                => intval($this->ci->input->get_post('sEcho')),
    ...
    protected function exec_replace($custom_val, $row_data)
    {
      ...
      if(isset($custom_val['replacement']) && is_array($custom_val['replacement']))
      {
        foreach($custom_val['replacement'] as $key => $val)
        {
          ...
          $custom_val['content'] = str_ireplace('$' . ($key + 1), auto_url($replace_string), $custom_val['content']); //inject auto encrypt URL (custom) = ID\primary key
        }
      }
    ...
    */
    ...
    {
      if(!$this->ci->input->get_post('mDataProp_0'))
        return FALSE;

      for($i = 0; $i < intval($this->ci->input->get_post('iColumns')); $i++)
        if(!is_numeric($this->ci->input->get_post('mDataProp_' . $i)))
          return TRUE;
    ...
    */
    protected function get_mDataprop()
    {
      ...
      for($i = 0; $i < intval($this->ci->input->get_post('iColumns')); $i++)
        $mDataProp[] = $this->ci->input->get_post('mDataProp_' . $i);
