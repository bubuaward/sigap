...
class Article extends ST_Controller {
	...
	public function index()
	{
		...
		$config = array(
			...
			'num_links'        => 5,
			'total_rows'       => $this->Master_mod->result_content(NULL, 0, array('content_type' => 'article')),
			'per_page'         => $limit,
		...
		$data = array(
			'meta_title' => translation('page_article_title'),
			'meta_description' => '',
			'meta_picture' => '',
			'meta_tags' => '',
			'alldata' => $this->Master_mod->result_content($limit, $offset, array('content_type' => 'article')),
		);
		
		$this->render(array('article/home'), $data, 0);
	}
	...
	{	
		...
		//Check data 404
		$detail = $this->Master_mod->detail_content_slug($slug, 'article');
		if(! $detail){
			$data = array(
			...
			$data = array(
				...
				'related' => $this->Master_mod->result_content(3, 0, array('content_type' => 'article', 'ex_content_id' => $detail->content_id)),
			);
