...
class Location extends ST_Controller {
	...
	public function home()
	{
		if($this->input->post('form_submit')){
			...
			if($this->form_validation->run()) {
				...
				create_function('$a', 'return $a["slug"];'),
					$this->db->like('slug', $slug_candidate)->select('slug')->get('master_location')->result_array()
				);
				...
				if($this->db->insert('master_location', $info)){
		...
		$this->display(array('main_header','location/home','main_footer'), $data, 0);
	}
	...
	public function edit($slug = null)
	{
		...
		$detail = $this->Master_mod->detail_master_location($cid);
		if(! $detail){
			$this->session->set_flashdata('msg', 'Request that you want is not available!');
			redirect($this->agent->referrer());
		}
		...
		if($this->input->post('form_submit')){
			...
			if($this->form_validation->run()) {
				...
				if($this->db->update('master_location', $info, array('location_id' => $cid))){
					$this->session->set_flashdata('msg', 'The process was successful');
				}else{
					$this->session->set_flashdata('msg', 'Change a few things up and try submitting again.');
				}
		...
		$this->display(array('main_header','location/edit','main_footer'), $data, 0);
	}
	...
	public function ajax_master_datatable()
	{
		...
		$this->datatables->select('location_id, location_name, created, slug');
		$this->datatables->unset_column('location_id');
		$this->datatables->add_column('actions', '$1', 'location_id');	
		$this->datatables->from('master_location');
		...
		foreach($data->aaData AS $red){
			...
			$action = '<div class="text-center">
				<a href="'.base_url(DIRADMIN.'/location/edit/'.$red->actions).'" class="btn bg-orange btn-xs btn-block" target="_self"><i class="fa fa-edit"></i> Edit</a>
			</div>';
			
			$newjson[] = array(
				'title' => $red->location_name,
				'slug' => $red->slug,
				'created' => $red->created,
				'actions' => $action
			);
