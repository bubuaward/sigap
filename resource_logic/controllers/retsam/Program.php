...
class Program extends ST_Controller {
	...
	public function home()
	{
		...
		$this->display(array('main_header','program/home','main_footer'), $data, 0);
	}
	...
	public function plus()
	{
		...
		if($this->input->post('form_submit')){
			...
			if($this->form_validation->run()) {
				...
				$info = array(
					...
					'geography_id' => $this->input->post('geography_id'),
		...
		$this->display(array('main_header','program/plus','main_footer'), $data, 0);
	}
	...
	public function edit($slug = null)
	{
		...
		$detail = $this->Master_mod->detail_content($cid);
		if(! $detail){
			$this->session->set_flashdata('msg', 'Request that you want is not available!');
			redirect(DIRADMIN.'/'.$this->router->fetch_class().'/home');
		}
		...
		$this->display(array('main_header','program/edit','main_footer'), $data, 0);
	}
	...
	public function media($slug = null)
	{
		...
		$this->display(array('main_header','program/gallery','main_footer'), $data, 0);
	}
	...
	public function ajax_master_datatable()
	{
		...
		$this->datatables->select('content.content_id AS content_id,content.slug AS slug, content.*');
		$this->datatables->select('master_geography.geography_name AS geography_name');
		$this->datatables->select('master_location.location_name AS location_name');
		$this->datatables->select("CONCAT('<img width=\"80px\" src=\"".base_url('storage/content')."/',content.cover,'\">') AS image", FALSE);
		$this->datatables->join('master_geography', 'master_geography.geography_id = content.geography_id', 'inner');
		$this->datatables->join('master_location', 'master_location.location_id = content.location_id', 'inner');
		$this->datatables->where('content.content_type', 'program');
		...
		foreach($data->aaData AS $red){
			...
			$action = '<div class="text-center">
				<a href="'.base_url(DIRADMIN.'/program/media/'.$red->actions).'" class="btn bg-black btn-xs btn-block" target="_self"><i class="fa fa-image"></i> Gallery</a>
				<a href="'.base_url(DIRADMIN.'/program/edit/'.$red->actions).'" class="btn bg-orange btn-xs btn-block" target="_self"><i class="fa fa-edit"></i> Edit</a>
				<a href="'.base_url(DIRADMIN.'/program/featured/'.$red->actions).'" class="btn bg-green btn-xs btn-block" target="_self"><i class="fa fa-star"></i> '.($red->is_featured == 'featured' ? 'UnFeatured' : 'Featured').'</a>
				<a onClick="return checkedbutton(\''.base_url(DIRADMIN.'/program/hide/'.$red->actions).'\')" href="javascript:;" class="btn bg-red btn-xs btn-block"><i class="fa fa-flag"></i> '.($red->published == 'publish' ? 'Unpublish' : 'Publish').'</a>
			</div>';
			...
			foreach($lang AS $sub){
				$html .= '<a href="'.base_url($sub->language_slug.'/program/'.$red->slug).'" target="_blank">'.$sub->title.'</a>
					<br/>
					<small>
						<span class="fa fa-map-marker"></span> '.$red->location_name.' 
						<span class="fa fa-globe"></span> '.$red->geography_name.' 
						<span class="fa fa-database"></span> Category: <a href="'.base_url($sub->language_slug.'/program/'.$cat->slug).'" target="_blank">'.$cat->title.'</a> 
					</small>
